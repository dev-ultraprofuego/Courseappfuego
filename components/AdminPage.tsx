...
const AdminPage: React.FC<AdminPageProps> = ({ onLogout, theme, onOpenCourseEditor, siteData, onDataChange, onSave, isSaving, lang, setLang }) => {
    ...
    const addLog = (actionCode: string, details: string) => {
        ...
        onDataChange({
            ...siteDataRef.current,
            auditLogs: [newLog, ...(siteDataRef.current.auditLogs || [])].slice(0, 200)
        });
    };
    ...
    const handleLoginSuccess = () => {
        ...
        setIsAuthenticated(true);
        addLog('LOGIN_SUCCESS', 'Admin|Système > Accès');
    };
    ...
    const handleCategoryCoursesSave = (categoryId: string, updatedLinks: CourseContextLink[]) => {
      ...
      addLog('CATEGORY_CONTENT', `Catégorie ${categoryId}|Catégories > ${categoryId} > Contenu`);
    };
    ...
    const handleSectionCoursesSave = (updatedSection: HomepageSection) => {
        ...
        addLog('SECTION_CONTENT', `${updatedSection.title}|Accueil > ${updatedSection.title} > Contenu`);
    };
    ...
    const handlePermanentDelete = (courseId: number) => {
        ...
        confirmAction(t.trashBin.deletePermanent, t.trashBin.irreversible, () => {
            const freshData = siteDataRef.current;
            
            const updatedCourses = freshData.coursesData.filter(c => c.id !== courseId);
            
            const newLog = createLogObject('COURSE_PURGE', `${courseTitle}|Système > Suppression Définitive`);
            const updatedLogs = [newLog, ...(freshData.auditLogs || [])].slice(0, 200);
    ...
    // Specific handler for Copy Link Log
    const handleLinkCopyLog = (fileName: string) => {
        addLog('LINK_COPY', `${fileName}|Médiathèque > Copie Lien`);
    };

    const handleMediaUpdate = (files: MediaFile[]) => {
        const freshData = siteDataRef.current;
        const currentFiles = freshData.mediaLibrary || [];
        let logAction = '';
        let logDetails = '';

        if (files.length > currentFiles.length) {
             const newFile = files[0]; // Assuming unshift/prepend
             logAction = 'MEDIA_UPLOAD';
             logDetails = `${newFile.name}|Médiathèque > Upload`;
        } else if (files.length < currentFiles.length) {
             // Find the missing file by comparing IDs safely (converting to string)
             const deletedFile = currentFiles.find(oldF => !files.some(newF => String(newF.id) === String(oldF.id)));
             const fileName = deletedFile ? deletedFile.name : 'Fichier';
             logAction = 'MEDIA_DELETE';
             logDetails = `${fileName}|Médiathèque > Suppression`;
        } else {
            // No change in count (e.g. reorder? not implemented yet)
            handleDataChange('mediaLibrary', files);
            return;
        }

        if (logAction) {
            const newLog = createLogObject(logAction, logDetails);
            onDataChange({
                ...freshData,
                mediaLibrary: files,
                auditLogs: [newLog, ...(freshData.auditLogs || [])].slice(0, 200)
            });
        }
    };
    ...
    if (!isAuthenticated) return <LoginScreen onLoginSuccess={handleLoginSuccess} theme={theme} />;
    ...
    return (
        <>
            <div className={`flex min-h-screen text-gray-200 bg-[#050505] font-sans selection:bg-white/20`}>
                {/* SIDEBAR */}
                <aside className="w-64 bg-[#0a0d0c] border-r border-white/5 flex-shrink-0 flex flex-col fixed h-full z-30 overflow-hidden">
                    <div className="h-20 flex items-center px-6 border-b border-white/5">
                        <h1 className="text-2xl font-black admin-one-effect flex items-center gap-2 tracking-tight cursor-pointer font-heading hover:opacity-80 transition-opacity" onClick={() => setActiveTab('dashboard')}>
                            CONTENT.<span className="admin-umbrella text-xl">☂️</span>
                        </h1>
                    </div>
                ...
                {/* MAIN CONTENT */}
                <main className="flex-1 ml-64 min-w-0 bg-[#050505] relative">
                    {/* Top Bar */}
                    <header className="h-20 bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-20 flex items-center justify-between px-8 transition-all duration-200">
                    ...
                    {/* Updated: Use min-h instead of fixed h to allow scrolling if content overflows, but fill screen if short */}
                    <div className={`w-full max-w-7xl mx-auto animate-fadeIn flex flex-col ${isFullPageTab ? 'min-h-[calc(100vh-5rem)] p-0' : 'h-full p-8 pb-20'}`}>
                        ...
                        {activeTab === 'testimonials' && <TestimonialsEditor testimonials={siteData.testimonials} onChange={val => handleDataChange('testimonials', val)} theme={theme} t={t} inputClass={adminInputStyles} textareaClass={adminTextareaStyles} confirmAction={confirmAction} />}
                        ...
                        {activeTab === 'media' && <MediaLibrary files={siteData.mediaLibrary || []} onUpdate={(files) => handleMediaUpdate(files)} theme={theme} confirmAction={confirmAction} notify={notify} onLog={handleLinkCopyLog} t={t} />}
