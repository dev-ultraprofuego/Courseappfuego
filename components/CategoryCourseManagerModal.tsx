...
const CategoryCourseManagerModal: React.FC<CategoryCourseManagerModalProps> = ({ isOpen, onClose, category, allCourses, categoryCourseLinks, onSave, onEditCourse, theme }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const coursesNotInsection = allCourses.filter(c => !courseIdsInCategory.has(c.id));
    ...
    const handleAddCourse = (courseId: number) => {
        ...
        if (courseToAdd) {
            setCoursesInCategory(prev => [...prev, { courseId, linkBehavior: 'detail', showBuyButton: false }]);
    ...
    const handleReorder = ({ dragIndex, hoverIndex }: ReorderPayload) => {
        const draggedItem = coursesInCategory[dragIndex];
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
            <div className={`w-full max-w-6xl h-[90vh] ${currentTheme.bg} border ${currentTheme.border} rounded-lg shadow-lg flex flex-col`}>
                <header className="p-4 border-b border-gray-700 flex justify-between items-center flex-shrink-0">
                    <h2 className={`text-2xl font-bold ${currentTheme.headerText}`}>Gérer les cours pour "{category.name}"</h2>
                    <button onClick={onClose} className="text-gray-400 hover:text-white text-3xl font-bold">&times;</button>
                </header>
                
                <div className="flex-1 grid grid-cols-2 gap-4 p-4 overflow-hidden">
                    <div className="flex flex-col">
                        <h3 className="font-semibold mb-2">Cours dans cette catégorie ({coursesInCategory.length})</h3>
...
const DraggableListItem: React.FC<{index: number, course: Course, linkConfig: CourseContextLink, onReorder: (payload: ReorderPayload) => void, onAction: (id: number) => void, onContextChange: (courseId: number, field: keyof CourseContextLink, value: any) => void, onEditCourse: (course: Course) => void, actionIcon: string, actionText: string, theme: Theme}> = ({ index, course, linkConfig, onReorder, onAction, onContextChange, onEditCourse, actionIcon, actionText, theme }) => {
    ...
    return (
         <div ref={ref} draggable onDragStart={handleDragStart} onDragOver={(e) => handleDragOver(e, index)} onDragEnd={() => dragItem.current = null} className="p-2 bg-gray-800 rounded-md">
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => onEditCourse(course)} title="Modifier le Cours" className="p-2 rounded-full text-blue-400 hover:bg-blue-900/50 flex-shrink-0">
            ...
            <div className="flex items-center gap-2 text-xs mt-2 pl-8 flex-wrap">
                ...
                {linkConfig.linkBehavior === 'external' && (
                    <input type="text" placeholder="URL Externe" value={linkConfig.overrideExternalLink || ''} onChange={e => onContextChange(course.id, 'overrideExternalLink', e.target.value)} className="bg-gray-700 p-1 rounded-md text-xs border border-gray-600 flex-grow min-w-[100px]" />
                )}
                <label className="flex items-center gap-1.5 cursor-pointer ml-2">
                    ...
                    Afficher bouton d'achat
                </label>
                 {linkConfig.showBuyButton && (
                    <input type="text" placeholder="URL du bouton d'achat" value={linkConfig.overrideBuyLink || ''} onChange={e => onContextChange(course.id, 'overrideBuyLink', e.target.value)} className="bg-gray-700 p-1 rounded-md text-xs border border-gray-600 flex-grow min-w-[100px]" />
                )}
            </div>
        </div>
    );
