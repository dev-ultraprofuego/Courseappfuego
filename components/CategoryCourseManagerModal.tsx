...
import type { Course, SoftwarePill, Theme, CategoryCourseManagerModalProps, ReorderPayload, CourseContextLink } from '../types';
import FeatherIcon from './FeatherIcon';

interface ExtendedCategoryCourseManagerModalProps extends CategoryCourseManagerModalProps {
    t: any;
    onLog?: (action: string, details: string) => void;
}

const CategoryCourseManagerModal: React.FC<ExtendedCategoryCourseManagerModalProps> = ({ isOpen, onClose, category, allCourses, categoryCourseLinks, onSave, onUpdateCategoryConfig, onEditCourse, theme, t, onLog }) => {
    const [coursesInCategory, setCoursesInCategory] = useState<CourseContextLink[]>(categoryCourseLinks);
    const [availableCourses, setAvailableCourses] = useState<Course[]>([]);
    const [searchTerm, setSearchTerm] = useState('');
    const [bulkUrl, setBulkUrl] = useState('');
    const [autoAdd, setAutoAdd] = useState(category.autoAdd || false);
    ...
    useEffect(() => {
        if (isOpen) {
            setIsClosing(false);
            setCoursesInCategory(categoryCourseLinks);
            setAutoAdd(category.autoAdd || false);
            const courseIdsInCategory = new Set(categoryCourseLinks.map(l => l.courseId));
            
            // Filter logic
            const coursesNotInsection = allCourses.filter(c => {
                const isNotAlreadyAdded = !courseIdsInCategory.has(c.id);
                const hasCategoryTag = category.category === 'all' || c.categories.some(cat => 
                    cat.toLowerCase().replace(/\s+/g, '-') === category.category
                );
                return isNotAlreadyAdded && hasCategoryTag;
            });
    ...
    }, [isOpen, category, allCourses, categoryCourseLinks]);
    ...
    const handleAddCourse = (courseId: number) => {
        ...
        if (courseToAdd) {
            setCoursesInCategory(prev => [{ courseId, linkBehavior: 'detail', showBuyButton: false }, ...prev]);
    ...
    const handleRemoveCourse = (courseId: number) => {
        ...
        if (courseToRemove) {
            ...
            setCoursesInCategory(prev => prev.filter(c => c.courseId !== courseId));
        }
    };
    ...
    const handleDragEnter = (index: number) => {
        ...
        if (dragItem.current !== null && dragItem.current !== index) {
            const newOrderedCourses = [...coursesInCategory];
            ...
            dragItem.current = index;
            setCoursesInCategory(newOrderedCourses);
        }
    };
    ...
    const handleContextChange = (courseId: number, field: keyof CourseContextLink, value: any) => {
        setCoursesInCategory(prev => prev.map(c => c.courseId === courseId ? { ...c, [field]: value } : c));
    };

    const handleSave = () => {
        if (autoAdd !== category.autoAdd && onLog) {
             const status = autoAdd ? 'activé' : 'désactivé';
             onLog('AUTO_ADD_UPDATE', `L'auto-ajout a été ${status} pour '${category.name}'|Catégories > ${category.name} > Gestion`);
        }
        onUpdateCategoryConfig(category.category, { autoAdd });
        onSave(category.category, coursesInCategory);
        handleClose();
    };
    
    const handleBulkApply = () => {
        if (!bulkUrl) return;
        setCoursesInCategory(prev => prev.map(c => ({
            ...c,
    ...
    // Common styles - Updated to Ghost Borders (border-white/10)
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0a0d0c]/80 backdrop-blur-md z-20 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${currentTheme.border} bg-white/5`}>
                            {category.iconType === 'feather' ? <FeatherIcon name={category.icon} size={18} className={currentTheme.accent}/> : <img src={category.icon} alt="" className="w-5 h-5"/>}
                        </div>
                        <div>
                            <h2 className="text-lg font-bold text-white tracking-tight">{t.manager.categoryTitle} {category.name}</h2>
                            <p className="text-xs text-gray-500">{t.manager.managePage}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden sm:flex items-center gap-3 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                            <div className="flex flex-col items-end mr-1">
                                ...
                                <span className="text-[9px] text-gray-600 italic leading-none">Tag: {category.category}</span>
                            </div>
                            <button 
                ...
                <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
                    ...
                    <div className="flex-1 flex flex-col border-b lg:border-b-0 lg:border-r border-white/5 bg-[#0a0d0c]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-[#0a0d0c]">
                             ...
                             {coursesInCategory.map((link, index) => {
...
const DraggableListItem: React.FC<{index: number, course: Course, linkConfig: CourseContextLink, onDragStart: (index: number) => void, onDragEnter: (index: number) => void, onDragEnd: () => void, onAction: (id: number) => void, onContextChange: (courseId: number, field: keyof CourseContextLink, value: any) => void, onEditCourse: (course: Course) => void, actionIcon: string, theme: Theme, inputStyle: string, selectStyle: string, t: any}> = ({ index, course, linkConfig, onDragStart, onDragEnter, onDragEnd, onAction, onContextChange, onEditCourse, actionIcon, theme, inputStyle, selectStyle, t }) => {
            ...
            <div className="flex items-center p-2 pl-0">
                ...
                <div className="flex-grow min-w-0 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                         ...
                         {linkConfig.showBuyButton && <span className="text-[9px] text-green-500/70 border border-green-500/20 px-1 rounded">Achat</span>}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-3">
                        ...
                        <div className="flex items-end pb-2">
                             <div className="flex items-center gap-2">
                                ...
                                />
                                <label htmlFor={`buy-${course.id}`} className="text-xs font-medium text-gray-300 select-none cursor-pointer">{t.manager.buyButton}</label>
                            </div>
                        </div>
                        ...
                        {linkConfig.showBuyButton && (
                             <div className="col-span-2 grid grid-cols-3 gap-2">
                                 <div className="col-span-2">
                                     ...
                                     <input type="text" placeholder="Défaut" value={linkConfig.overrideBuyLink || ''} onChange={e => onContextChange(course.id, 'overrideBuyLink', e.target.value)} className={inputStyle} />
                                 </div>
...
export default CategoryCourseManagerModal;