...
const CourseCarousel: React.FC<CourseCarouselProps> = ({ section, allCourses, pills, theme, config }) => {
    ...
    const themeClasses = {
        iris: {
            ...
            paginationActive: "bg-[#7de8d0]",
            paginationInactive: "bg-[rgba(93,213,184,0.2)]",
            navButtonBg: "bg-[rgba(93,213,184,0.06)]",
            navButtonBorder: "border-[rgba(93,213,184,0.12)]",
            navButtonText: "text-[#7de8d0]",
            navButtonHover: "hover:bg-[rgba(93,213,184,0.15)]",
        },
        pristine: {
            titleBg: "bg-gradient-to-r from-[rgba(255,71,87,0.18)] to-[rgba(255,107,129,0.13)]",
            titleBorder: "border-[rgba(255,71,87,0.18)]",
            paginationActive: "bg-[#ff6b81]",
            paginationInactive: "bg-[rgba(255,71,87,0.2)]",
            navButtonBg: "bg-[rgba(255,71,87,0.06)]",
            navButtonBorder: "border-[rgba(255,71,87,0.12)]",
            navButtonText: "text-[#ff6b81]",
            navButtonHover: "hover:bg-[rgba(255,71,87,0.15)]",
        },
        boss: {
            titleBg: "bg-gradient-to-r from-[rgba(217,38,38,0.18)] to-[rgba(242,68,68,0.13)]",
            titleBorder: "border-[rgba(217,38,38,0.18)]",
            paginationActive: "bg-[#F24444]",
            paginationInactive: "bg-[rgba(217,38,38,0.2)]",
            navButtonBg: "bg-[rgba(217,38,38,0.06)]",
            navButtonBorder: "border-[rgba(217,38,38,0.12)]",
            navButtonText: "text-[#F24444]",
            navButtonHover: "hover:bg-[rgba(217,38,38,0.15)]",
        }
    }
                ...
                <div className="flex flex-col justify-center items-center gap-4 mt-8">
                    <div className="flex gap-2">
                        {Array.from({ length: totalPages }).map((_, index) => (
                            <button key={index} onClick={() => setCurrentPage(index)} className={`h-2.5 rounded-full cursor-pointer border-none p-0 transition-all duration-300 ${currentPage === index ? `w-8 ${currentTheme.paginationActive}` : `w-2.5 ${currentTheme.paginationInactive}`}`}></button>
                        ))}
                    </div>
                    <div className="flex items-center gap-2">
                        <button 
                            ...
                            aria-label="Previous"
                            className={`border w-12 h-12 rounded-[10px] cursor-pointer flex items-center justify-center transition-all duration-300 ${currentTheme.navButtonBg} ${currentTheme.navButtonBorder} ${currentTheme.navButtonText} ${isPrevDisabled ? disabledClasses : currentTheme.navButtonHover}`}
                        >
                        ...
                        <button 
                            ...
                            aria-label="Next"
                            className={`border w-12 h-12 rounded-[10px] cursor-pointer flex items-center justify-center transition-all duration-300 ${currentTheme.navButtonBg} ${currentTheme.navButtonBorder} ${currentTheme.navButtonText} ${isNextDisabled ? disabledClasses : currentTheme.navButtonHover}`}
                        >
