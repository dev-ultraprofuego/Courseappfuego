...
import type { Course, Theme, SiteConfig } from '../types';
...
const CourseDetailPage: React.FC<CourseDetailPageProps> = ({ course, allCourses, theme, showBuyButton, buyLink, buyButtonText, config }) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div className="lg:col-span-8">
                    ...
                    <div className="text-sm text-white/60 mb-4">
                        ...
                        <Link href={`/category/${course.categories[0].toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-white transition-colors">{course.categories[0]}</Link>
                        <span className="mx-2">&rsaquo;</span>
                        ...
                        <div className="space-y-6">
                            {recentPosts.map(post => (
                                <Link key={post.id} href={`/course/${post.id}`} className="flex items-center gap-4 group cursor-pointer">
                                    ...
                                    <div>
                                        <span className={`text-xs font-bold ${theme === 'iris' ? 'text-[#7de8d0]' : 'text-[#F24444]'}`}>{post.categories[0]}</span>
