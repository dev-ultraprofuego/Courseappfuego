    ...
    const renderMobileDropdown = (data: NavDropdown, name: string) => {
        ...
        return (
            <div className="mb-2">
                ...
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${currentTheme.bgMobile} rounded-lg mt-2 ${isOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
                    {name === 'theme' ? (
                        [
                            { title: config.themeNameIris, key: 'iris' as Theme, icon: 'droplet' },
                            { title: config.themeNamePristine, key: 'pristine' as Theme, icon: 'droplet' }
        ...
        <>
            ...
            <header className={`bg-[#0d1614] border-b ${currentTheme.border} sticky top-0 z-40 transition-shadow duration-300 ${scrolled ? 'shadow-[0_2px_20px_rgba(0,0,0,0.3)]' : ''}`}>
                <div className="max-w-[1400px] mx-auto px-4 sm:px-8 flex items-center justify-between h-20">
                    <Link href="/" className="no-underline text-4xl font-black bg-transparent border-none cursor-pointer p-0 inline-flex items-baseline">
                    ...
                    <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`lg:hidden flex flex-col gap-[5px] ${currentTheme.bgMobile} border ${currentTheme.border} p-2.5 rounded-lg cursor-pointer`}>
                        <span className={`w-6 h-0.5 ${theme === 'iris' ? 'bg-[#7de8d0]' : 'bg-[#F24444]'} rounded-sm transition-all duration-300 ${isMenuOpen ? `rotate-45 translate-y-[7px]` : ''}`}></span>
                        <span className={`w-6 h-0.5 ${theme === 'iris' ? 'bg-[#7de8d0]' : 'bg-[#F24444]'} rounded-sm transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
                        <span className={`w-6 h-0.5 ${theme === 'iris' ? 'bg-[#7de8d0]' : 'bg-[#F24444]'} rounded-sm transition-all duration-300 ${isMenuOpen ? `-rotate-45 -translate-y-[7px]` : ''}`}></span>
                    </button>
                </div>
            </header>
            
            <div className={`fixed top-0 left-0 w-full h-full bg-[#0d1614] z-30 transition-transform duration-500 ease-in-out lg:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-8 mt-20">
                    <Link href={premiumLinkHref} onClick={handleLinkClick} className="block w-full text-left p-4 text-white/90 bg-transparent border-none font-semibold text-base rounded-lg">{navData.premium.title}</Link>
                    ...
                    <button onClick={handleSearchClick} className="block w-full text-left p-4 text-white/90 bg-transparent border-none font-semibold text-base rounded-lg">Search</button>
                </div>
            </div>
        </>
    );
