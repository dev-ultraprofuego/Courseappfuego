...
import type { PricingPageProps, PricingTier } from '../types';
...
const PricingPage: React.FC<PricingPageProps> = ({ theme, tiers }) => {
    ...
    const cardStyles = theme === 'iris' ? {
        '--mint': '#66a68e',
        '--mint-light': '#74b49b',
        '--check': '#28c76f',
        '--cta-border': 'rgba(116,180,155,0.35)',
        '--popular-border': 'rgba(116,180,155,0.18)',
    } as React.CSSProperties : {
        '--mint': '#D92626',
        '--mint-light': '#F24444',
        '--check': '#F24444',
        '--cta-border': 'rgba(217,38,38,0.35)',
        '--popular-border': 'rgba(217,38,38,0.18)',
    } as React.CSSProperties;
        ...
        <div className={`bg-gradient-to-b from-[#0f1513] to-[#070707] text-white font-['Poppins',_sans_serif] py-12 ${theme}`}>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-center my-10">
                    <div ref={switchRef} className="relative bg-[rgba(15,20,19,0.6)] border border-white/10 rounded-full p-1.5 inline-flex gap-2 backdrop-blur-md shadow-[0_4px_16px_rgba(0,0,0,0.4)] flex-wrap justify-center">
                        ...
                        {tiers.map(tier => (
                             <button key={tier.id} className={`toggle-option py-3 px-6 rounded-full font-semibold text-base cursor-pointer transition-colors duration-400 border-none bg-transparent flex items-center gap-2.5 relative z-10 ${activeSection === tier.id ? 'text-white active' : 'text-white/60 hover:text-white/90'}`} data-section={tier.id} onClick={() => handleToggle(tier.id)}>
                              <span>{tier.name}</span>
                            </button>
                        ))}
                    </div>
...
const pricingStyles = `
    ...
    .pristine .feature-list li::before {
         text-shadow: 0 0 6px rgba(217,38,38,0.5);
    }
    ...
    .iris .animate-pulse-glow {
        animation: pulse-glow-iris 3s infinite ease-in-out;
    }
    .pristine .animate-pulse-glow {
        animation: pulse-glow-pristine 3s infinite ease-in-out;
    }
    .iris .active-pill {
    ...
    .pristine .card.popular {
        background-image: linear-gradient(to bottom, rgba(45,22,25,0.55), rgba(0,0,0,0.4));
    }
    .iris .card.popular {
